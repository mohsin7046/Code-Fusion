...
function ShortList_Job() {
    ...
    const fetchVerifyData = async () => {
      try {
        const res = await fetch('/api/recruiter/all-interviews-data', {
          ...
          body: JSON.stringify({ id: formData.userId, status: "UNDER_REVIEW" }),
        });
  ...
  const handleNavigate = (index,id) => {
    navigate(`/dashboard/shortlist-details/${id}`);
  };
  ...
  return (
    <div className="p-6 text-black">
      <div className="flex flex-row items-center font-semibold text-3xl self-start">
        ...
        />
        <h2 className="text-2xl font-semibold mb-4 ml-2">Shortlist Candidate</h2>
      </div>
