    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Set available test types
                setAvailableTests({
                    onlineTest: result?.hasOnlineTest || false,
                    behaviouralTest: result?.hasAIInterview || false,
                    codingTest: result?.hasCodingTest || false
                });
                ...
                setTests(prev => ({
                    ...prev,
                    onlineTest: {
                        date: formatDateForInput(data?.onlineTestDate),
                        saved: !!data?.onlineTestDate
                    },
                    behaviouralTest: {
                        date: formatDateForInput(data?.behavioralInterviewDate),
                        saved: !!data?.behavioralInterviewDate
                    },
                    codingTest: {
                        date: formatDateForInput(data?.codingTestDate),
                        saved: !!data?.codingTestDate
                    }
                }));
        ...
        <div className='flex justify-center items-center w-screen h-screen'>
            <div className="max-w-6xl p-4 bg-gray-100 rounded-md shadow">
                ...
                <div className="space-y-4">
                    {testConfigs.filter(config => availableTests[config.key]).map((config, idx) => {
                ...
                <div className="mt-6 p-3 bg-white border border-gray-300 rounded-md">
                    ...
                    <p className="text-sm text-gray-600">
                        Scheduled Tests: {Object.values(tests).filter(t => t.saved).length} / {Object.values(availableTests).filter(Boolean).length}
                    </p>
                </div>
            </div>
        </div>
